...
<input type="hidden" name="post[mycatid]" id="mycatid" value="<?php echo $mycatid;?>"/>
...
<tr>
<td class="tl"><span class="f_red">*</span> 祖先分类</td>
<td><input readonly="readonly" type="text" id="mycatid1" size="60" value="<?php echo $mycatid;?>"/> <span style="padding: 5px 7px;background: #4d8eef;color: #fff;
    border-radius: 5px;" onclick="getmycatid()">获取产品一级分类</span>  注：必选，可以当做首页推荐公司必备属性</td>
</tr>
<tr>
...
<td><input name="post[brand]" readonly="readonly" type="text" size="30" value="<?php echo $brand!=Null?$brand:$_company;?>"/></td>
</tr>
<?php if($CP) { ?>
...
<tr>
<td class="tl"><span class="f_hid">*</span> 是否推荐<span style="color: darkred;">优质货源</span></td>
<td>
...
<p style="color: darkred;">如果推荐为优质货源，则会被推荐至首页优质货源模块</p>
<tr>
...
<script type="text/javascript">
function getmycatid(){
	var mycatid = $("#load_category_1 select:first").attr("data-index");
	if(mycatid == 0){
		Dmsg('请选择商品分类', 'catid', 1);
	}else{
		$("#mycatid").val(mycatid);
		$("#mycatid1").val(mycatid);
	}
}
function _p() {
	if(Dd('tag').value) {
