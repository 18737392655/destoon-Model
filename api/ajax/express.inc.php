...
if($DT['cloud_express'] && DT_CLOUD_UID && DT_CLOUD_KEY) {
	...
	if(is_file($cid)) {
		...
		if($_com == '顺丰') {
			if($from == 'mall') {
				$t = $db->get_one("SELECT buyer_mobile FROM {$DT_PRE}order WHERE itemid=$itemid");
				if($t && $t['buyer_mobile']) $mobile = $t['buyer_mobile'];
			} else if($from == 'group') {
				if($mid && $MODULE[$mid]['module'] == $from) {
					$t = $db->get_one("SELECT buyer_mobile FROM {$DT_PRE}{$from}_order_{$mid} WHERE itemid=$itemid");
					if($t && $t['buyer_mobile']) $mobile = $t['buyer_mobile'];
				}
			}
		}
		$rec = dcloud('express->com='.$_com.'&no='.$no.'&mobile='.$mobile);
	}
