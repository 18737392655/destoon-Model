...
<?php if($action == 'view') { ?><td class="tab" id="view"><a href="?action=view&chatid=<?php echo $chatid;?>"><span>聊天记录</span></a></td><?php } ?>
...
<tr>
<td class="tl">黑名单</td>
<td class="tr f_gray"><textarea name="black" id="black" style="width:90%;height:60px;overflow:visible;"><?php echo $user['black'];?></textarea><br/>直接输入会员名即可将会员列入黑名单，多个会员名请用空格键隔开</td>
</tr>
<tr>
<td class="tl">自动回复</td>
<td class="tr f_gray"><textarea name="reply" id="reply" style="width:90%;height:60px;overflow:visible;"><?php echo $user['reply'];?></textarea><br/>会员首次发起交谈或超过一周没有交谈时，自动回复内容</td>
</tr>
<tr>
<td class="tl"> </td>
...
<input type="button" value=" 重 搜 " class="btn" onclick="Go('?action=<?php echo $action;?>&chatid=<?php echo $chatid;?>');"/>
</div>
</form>
<div class="ls">
...
<td align="left"><?php echo $v['word'];?></td>
</tr>
<?php } } ?>
</table>
</div>
...
<script type="text/javascript">
function Lchat() {
$('#chat-list').load('?action=list&reload=<?php echo $DT_TIME;?>', function() {
