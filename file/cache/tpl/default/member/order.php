...
<script type="text/javascript">
var amount = 0;
var itemids = [<?php if(is_array($lists)) { foreach($lists as $i => $t) { ?><?php if($i) { ?>,<?php } ?>'<?php echo $t['itemid'];?>'<?php } } ?>];
...
<td class="<?php if($nav==0) { ?>nav_2<?php } else { ?>nav_1<?php } ?>"><a href="?nav=0">待确认 <span>(<?php echo $db->count($table, "buyer='$_username' AND status=0");?>)</span></a></td>
<td class="<?php if($nav==1) { ?>nav_2<?php } else { ?>nav_1<?php } ?>"><a href="?nav=1">待付款 <span>(<?php echo $db->count($table, "buyer='$_username' AND status=1");?>)</span></a></td>
<td class="<?php if($nav==2) { ?>nav_2<?php } else { ?>nav_1<?php } ?>"><a href="?nav=2">待发货 <span>(<?php echo $db->count($table, "buyer='$_username' AND status=2");?>)</span></a></td>
<td class="<?php if($nav==3) { ?>nav_2<?php } else { ?>nav_1<?php } ?>"><a href="?nav=3">待收货 <span>(<?php echo $db->count($table, "buyer='$_username' AND status=3");?>)</span></a></td>
<td class="<?php if($nav==5) { ?>nav_2<?php } else { ?>nav_1<?php } ?>"><a href="?nav=5">待退款 <span>(<?php echo $db->count($table, "buyer='$_username' AND status=5");?>)</span></a></td>
<td class="<?php if($nav==6) { ?>nav_2<?php } else { ?>nav_1<?php } ?>"><a href="?nav=6">已退款 <span>(<?php echo $db->count($table, "buyer='$_username' AND status=6");?>)</span></a></td>
<td class="<?php if($nav==4) { ?>nav_2<?php } else { ?>nav_1<?php } ?>"><a href="?nav=4">待评价 <span>(<?php echo $db->count($table, "buyer='$_username' AND status=4 AND seller_star=0");?>)</span></a></td>
<td>&nbsp;</td>
</tr>
</table>
</div>
...
<td width="100"><div style="margin-bottom:15px;"><a href="<?php echo userurl($v['seller'], 'file=contact');?>" target="_blank"><?php echo $v['seller'];?></a></div><?php if($DT['im_web']) { ?><?php echo im_web($v['seller'].'&mid=16&itemid='.$v['mallid']);?> <?php } ?><a href="message.php?action=send&touser=<?php echo $v['seller'];?>"><img src="image/ico_message.gif" title="发送站内信" align="absmiddle"/></a></td>
<td width="120">
<?php echo $v['dstatus'];?>
<?php if($v['status'] == 1) { ?>
