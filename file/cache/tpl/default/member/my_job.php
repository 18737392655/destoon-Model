...
<?php for($i = 1; $i < 21; $i++) { ?>
<option value="<?php echo $i;?>"<?php if($i == $experience) { ?> selected<?php } ?>><?php echo $i;?></option>
<?php } ?>
</select> &nbsp;&nbsp;年以上</td>
</tr>
<?php if($CP) { ?>
...
<td class="tr">
<input type="radio" name="post[sex]" value="1"<?php if($sex == 1) { ?> checked="checked"<?php } ?>/> 先生
<input type="radio" name="post[sex]" value="2"<?php if($sex == 2) { ?> checked="checked"<?php } ?>/> 女士
</td>
</tr>
<tr>
...
<input type="checkbox" name="thumb" id="thumb" value="1"<?php if($thumb) { ?> checked<?php } ?>/> 有照片&nbsp;
</div>
<div class="tt">
<select name="type">
<?php if(is_array($TYPE)) { foreach($TYPE as $k => $v) { ?>
...
<tr align="center">
<td><a href="<?php echo $v['linkurl'];?>" target="_blank"><img src="<?php if($v['thumb']) { ?><?php echo $v['thumb'];?><?php } else { ?><?php echo DT_SKIN;?>image/nohead.gif<?php } ?>" width="60" alt=""/></a></td>
<td><a href="<?php echo $v['linkurl'];?>" target="_blank" class="t"><?php echo $v['truename'];?></a></td>
<td><?php if($v['gender']==1) { ?>男<?php } else { ?>女<?php } ?></td>
...
<td><?php if($v['marriage']==2) { ?>已婚<?php } else { ?>未婚<?php } ?></td>
<td><?php if($v['experience']) { ?><?php echo $v['experience'];?>年<?php } else { ?>无<?php } ?></td>
...
<td><a href="?mid=<?php echo $mid;?>&action=resume_show&itemid=<?php echo $v['applyid'];?>&resumeid=<?php echo $v['resumeid'];?>&jobid=<?php echo $v['jobid'];?>" target="_blank" title="点击查看简历"><?php if($v['status'] == 1) { ?><span class="f_red">未查看</span><?php } else { ?>已查看<?php } ?></a></td>
<td><a href="?mid=<?php echo $mid;?>&action=resume_invite&itemid=<?php echo $v['applyid'];?>&resumeid=<?php echo $v['resumeid'];?>&jobid=<?php echo $v['jobid'];?>" target="_blank" title="点击邀请面试"><?php if($v['status'] == 3) { ?>已邀请<?php } else { ?><span class="f_red">未邀请</span><?php } ?></a></td>
...
<td><a href="?mid=<?php echo $mid;?>&action=resume&itemid=<?php echo $v['itemid'];?>"><span class="<?php if($v['apply']) { ?>f_red<?php } else { ?>f_gray<?php } ?>"><?php echo $v['apply'];?></span></a></td>
<td>
