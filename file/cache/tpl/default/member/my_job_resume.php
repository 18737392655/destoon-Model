...
<tr>
<td class="tab" id="add"><a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=add"><span>创建简历</span></a></td>
<?php if($_userid) { ?>
<td class="tab" id="s3"><a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>"><span>已发布(<?php echo $nums['3'];?>)</span></a></td>
<td class="tab" id="s2"><a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&status=2"><span>审核中(<?php echo $nums['2'];?>)</span></a></td>
<td class="tab" id="s1"><a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&status=1"><span>未通过(<?php echo $nums['1'];?>)</span></a></td>
<td class="tab" id="apply"><a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=apply"><span>应聘职位(<?php echo $nums['apply'];?>)</span></a></td>
<?php } ?>
</tr>
</table>
</div>
...
<?php if($k > 0) { ?>
<input type="radio" name="post[gender]" id="gender_<?php echo $k;?>" value="<?php echo $k;?>"<?php if($k == $gender) { ?> checked<?php } ?>/><label for="gender_<?php echo $k;?>"> <?php echo $v;?></label> 
<?php } ?>
<?php } } ?>
</td>
</tr>
...
<?php if($k > 0) { ?>
<input type="radio" name="post[marriage]" id="marriage_<?php echo $k;?>" value="<?php echo $k;?>"<?php if($k == $marriage) { ?> checked<?php } ?>/><label for="marriage_<?php echo $k;?>"> <?php echo $v;?></label> 
<?php } ?>
<?php } } ?>
</td>
</tr>
...
<?php for($i = 1; $i < 13; $i++) { ?>
<option value="<?php echo $i;?>"<?php if($i == $bmonth) { ?> selected<?php } ?>><?php echo $i;?></option>
<?php } ?>
</select>
月
<select name="post[bday]">
<?php for($i = 1; $i < 32; $i++) { ?>
<option value="<?php echo $i;?>"<?php if($i == $bday) { ?> selected<?php } ?>><?php echo $i;?></option>
<?php } ?>
</select>
日
...
<?php if($k > 0) { ?>
<input type="radio" name="post[education]" id="education_<?php echo $k;?>" value="<?php echo $k;?>"<?php if($k == $education) { ?> checked<?php } ?>/><label for="education_<?php echo $k;?>"> <?php echo $v;?></label> 
<?php } ?>
<?php } } ?>
</td>
</tr>
...
<?php if($k > 0) { ?>
<input type="radio" name="post[type]" id="type_<?php echo $k;?>" value="<?php echo $k;?>"<?php if($k == $type) { ?> checked<?php } ?>/><label for="type_<?php echo $k;?>"> <?php echo $v;?></label> 
<?php } ?>
<?php } } ?>
</td>
</tr>
...
<option value="<?php echo $k;?>"<?php if($k==$situation) { ?> selected<?php } ?>><?php echo $v;?></option> 
<?php } } ?>
</select>
</td>
</tr>
...
<td class="tl">公开程度</td>
<td class="tr">
<input type="radio" name="post[open]" value="3"<?php if($open==3) { ?> checked<?php } ?>/> 开放
<input type="radio" name="post[open]" value="2"<?php if($open==2) { ?> checked<?php } ?>/> 仅网站可见
<input type="radio" name="post[open]" value="1"<?php if($open==1) { ?> checked<?php } ?>/> 关闭
</td>
</tr>
<tr>
<td class="tl">简历模板</td>
...
<?php if($action=='add') { ?>
<script type="text/javascript">s('resume_<?php echo $mid;?>');m('<?php echo $action;?>');</script>
<?php } else { ?>
<script type="text/javascript">s('resume_<?php echo $mid;?>');m('s<?php echo $status;?>');</script>
<?php } ?>
...
<input type="button" value=" 重 置 " class="btn" onclick="Go('?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=<?php echo $action;?>');"/>
</form>
</div>
<div class="ls">
...
<td class="f_gray"><?php echo timetodate($v['applytime'], 5);?></td>
<td>&nbsp;&nbsp;
<?php if($v['status'] == 3) { ?>
...
<td>
<a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=apply_delete&itemid=<?php echo $v['applyid'];?>" onclick="return confirm('确定要删除吗？此操作将不可撤销');"><img width="16" height="16" src="image/delete.png" title="删除" alt=""/></a>
</td>
</tr>
<?php } } ?>
</table>
...
<script type="text/javascript">s('resume_<?php echo $mid;?>');m('apply');</script>
<?php } else { ?>
<div class="tt">
<form action="?">
<input type="hidden" name="mid" value="<?php echo $mid;?>"/>
...
<?php if(is_array($lists)) { foreach($lists as $k => $v) { ?>
<tr align="center">
<td align="left">&nbsp;&nbsp;&nbsp;<?php if($v['status']==3) { ?><a href="<?php echo $v['linkurl'];?>" target="_blank" class="t"><?php } else { ?><a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=edit&itemid=<?php echo $v['itemid'];?>" class="t"><?php } ?><?php echo $v['title'];?></a><?php if($v['status']==1 && $v['note']) { ?> <a href="javascript:" onclick="alert('<?php echo $v['note'];?>');"><img src="image/why.gif" title="未通过原因"/></a><?php } ?></td>
...
<a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=add&itemid=<?php echo $v['itemid'];?>&catid=<?php echo $v['catid'];?>"><img width="16" height="16" src="image/new.png" title="复制简历" alt=""/></a>&nbsp;
<a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=refresh&itemid=<?php echo $v['itemid'];?>"><img width="16" height="16" src="image/refresh.png" title="一键更新" alt=""/></a>&nbsp;
<a href="?mid=<?php echo $mid;?>&job=<?php echo $job;?>&action=delete&itemid=<?php echo $v['itemid'];?>" onclick="return confirm('确定要删除吗？此操作将不可撤销');"><img width="16" height="16" src="image/delete.png" title="删除" alt=""/></a>
</td>
</tr>
<?php } } ?>
</table>
</div>
<?php if($resume_limit || (!$MG['fee_mode'] && $MOD['fee_add'])) { ?>
<div class="limit">
<?php if($resume_limit) { ?>
总共可发 <span class="f_b f_red"><?php echo $resume_limit;?></span> 条&nbsp;&nbsp;&nbsp;
...
可免费发布 <span class="f_b"><?php if($resume_free_limit<0) { ?>无限<?php } else { ?><?php echo $resume_free_limit;?><?php } ?></span> 条&nbsp;&nbsp;&nbsp;
<?php } ?>
</div>
<?php } ?>
<div class="pages"><?php echo $pages;?></div>
