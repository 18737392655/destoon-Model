...
<td class="tab" id="s3"><a href="?action=index"><span>已发布(<?php echo $nums['3'];?>)</span></a></td>
<td class="tab" id="s2"><a href="?status=2"><span>审核中(<?php echo $nums['2'];?>)</span></a></td>
<td class="tab" id="s1"><a href="?status=1"><span>未通过(<?php echo $nums['1'];?>)</span></a></td>
<td class="tab" id="type"><a href="javascript:Dwidget('type.php?item=news', '[新闻分类]', 600, 300);"><span>新闻分类(<?php echo $nums['0'];?>)</span></a></td>
</tr>
</table>
</div>
...
<tr>
<td class="tl">&nbsp;</td>
<td class="tr" height="50"><input type="submit" name="submit" value="<?php if($action == 'edit') { ?>修 改<?php } else { ?>添 加<?php } ?>" class="btn_g"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="取 消" class="btn" onclick="history.back(-1);"/></td>
</tr>
</table>
</form>
<?php echo load('clear.js');?>
<script type="text/javascript">s('news');m(<?php if($action=='add') { ?>'add'<?php } else { ?>'s<?php echo $status;?>'<?php } ?>);</script>
<?php } else { ?>
<form action="?">
...
<td height="30" align="left">&nbsp;&nbsp;&nbsp;<?php if($status==3) { ?><a href="<?php echo $v['linkurl'];?>" class="t" target="_blank"><?php } else { ?><a href="?action=edit&itemid=<?php echo $v['itemid'];?>" class="t"><?php } ?><?php echo $v['title'];?></a><?php if($v['status']==1 && $v['note']) { ?> <a href="javascript:" onclick="alert('<?php echo $v['note'];?>');"><img src="image/why.gif" title="未通过原因"/></a><?php } ?></td>
...
<td><a href="?action=delete&itemid=<?php echo $v['itemid'];?>" onclick="if(!confirm('确定要删除吗？此操作将不可撤销')) return false;"><img width="16" height="16" src="image/delete.png" title="删除" alt=""/></a></td>
</tr>
<?php } } ?>
</table>
</div>
