...
<img src="<?php echo DT_SKIN;?>image/btn_tobuy.gif" alt="购买" class="c_p" onclick="Go('<?php echo $MODULE['2']['linkurl'];?>buy.php?mid=<?php echo $moduleid;?>&itemid=<?php echo $itemid;?>');"/>
&nbsp;
<img src="<?php echo DT_SKIN;?>image/btn_addcart.gif" alt="加入购物车" class="c_p" onclick="Go('<?php echo $MODULE['2']['linkurl'];?>cart.php?action=add&mid=<?php echo $moduleid;?>&itemid=<?php echo $itemid;?>');"/>
<?php } else { ?>
...
<?php } ?>
<?php if($username) { ?>
<div class="head-txt"><span><a href="<?php echo userurl($username, 'file=sell');?>" target="_blank">更多<i>&gt;</i></a></span><strong>本企业其它产品</strong></div>
<div class="list-thumb"><?php echo tag("moduleid=$moduleid&length=20&condition=status=3 and thumb<>'' and username='$username'&pagesize=8&order=edittime desc&width=100&height=100&cols=8&template=thumb-table");?></div>
<?php } else { ?>
