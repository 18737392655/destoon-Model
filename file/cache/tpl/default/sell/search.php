...
<input type="checkbox" name="price" id="price" value="1"<?php if($price) { ?> checked<?php } ?>/>标价 &nbsp;
<input type="checkbox" name="thumb" id="thumb" value="1"<?php if($thumb) { ?> checked<?php } ?>/>图片 &nbsp;
...
<ul>
<li<?php if($typeid==99) { ?> class="on"<?php } ?>><a href="###" onclick="Dd('typeid').value='99';Dd('fsearch').submit();">全部</a></li>
<?php if(is_array($TYPE)) { foreach($TYPE as $k => $v) { ?>
<li<?php if($typeid==$k) { ?> class="on"<?php } ?>><a href="###" onclick="Dd('typeid').value='<?php echo $k;?>';Dd('fsearch').submit();"><?php echo $v;?></a></li>
<?php } } ?>
</ul>
</div>
<div class="c_b"></div>
