...
switch($action) {
	...
	case 'main':
		if($submit) {
			...
			$notice_url = base64_decode('aHR0cHM6Ly93d3cuZGVzdG9vbi5jb20vY2xpZW50LnBocD9hY3Rpb249bm90aWNlJnByb2R1Y3Q9YjJiJnZlcnNpb249').DT_VERSION.'&release='.DT_RELEASE.'&lang='.DT_LANG.'&charset='.DT_CHARSET.'&domain='.DT_DOMAIN.'&install='.$install.'&os='.PHP_OS.'&soft='.urlencode($_SERVER['SERVER_SOFTWARE']).'&php='.urlencode(phpversion()).'&mysql='.urlencode($db->version()).'&url='.urlencode($DT_URL).'&site='.urlencode($DT['sitename']).'&auth='.strtoupper(md5($DT_URL.$install.$_SERVER['SERVER_SOFTWARE']));
			$install = timetodate($install, 5);			
			$edition = edition(1);
			include tpl('main');
			if($_founder) {
				echo '<script type="text/javascript" src="'.$notice_url.'"></script>';
				echo '<script type="text/javascript">';
				echo '$(function(){';
				echo '	var destoon_release = '.DT_RELEASE.';';
				echo '	var destoon_version = '.DT_VERSION.';';
				echo '	if(typeof destoon_lastrelease != "undefined") {';
				echo '		var lastrelease = parseInt(destoon_lastrelease.replace("-", "").replace("-", ""));';
				echo '		if(destoon_lastversion == destoon_version && destoon_release < lastrelease) {';
				echo '			$("#last_v").html(destoon_lastversion);';
				echo '			$("#last_r").html(destoon_lastrelease);';
				echo '			$("#tips_update").slideDown(600);';
				echo '		}';
				echo '	}';
				echo '});';
				echo '</script>';
			}
			echo '<script type="text/javascript">Menuon(0);</script>';
			include tpl('footer');
		}
	break;
	case 'left':
